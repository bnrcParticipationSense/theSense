...
public class Collection implements SensorEventListener {
	...
	/*
	 * private Collection() { Log.i("new Collection()",
	 * "new Collection @ Activity = "+this.app.getApplicationInfo().toString());
	 * 
	 * //sensorManager = (SensorManager)
	 * this.app.getSystemService(android.content.Context.SENSOR_SERVICE);
	 * //connectManager = (ConnectivityManager)
	 * this.app.getSystemService(Context.CONNECTIVITY_SERVICE);
	 * //mLocationClient = new LocationClient(this.app.getApplicationContext());
	 * // //mLocationClient = new LocationClient(this.app); sensorManager =
	 * (SensorManager)
	 * this.app.getSystemService(android.content.Context.SENSOR_SERVICE);
	 * connectManager = (ConnectivityManager)
	 * this.app.getSystemService(Context.CONNECTIVITY_SERVICE); mLocationClient
	 * = new LocationClient(this.app);
	 * 
	...
	private Collection(Activity app) {
		...
		Log.i("new Collection()", "new Collection @ Activity = "
				+ this.app.getApplicationInfo().toString());

		sensorManager = (SensorManager) this.app
				.getSystemService(android.content.Context.SENSOR_SERVICE);
		connectManager = (ConnectivityManager) this.app
	...
	private void setValues() {
		...
		// runForNoise();
		/*
		 * Thread noise_t = new Thread() { public void run(){ while(true){
		 * if(need_noise) { runForNoise(); need_noise = false; } }
		 * 
		 * } };
		 */
		// noise_t.start();
		...
		// int bs =
		// this.app.getIntent().getIntExtra(BatteryManager.EXTRA_STATUS, -1);
	...
	private void calculateOrientation() {
		...
		// Log.i("calculateOrientation",
		// "collect.orientation = "+orientation[0]+" , "+orientation[1]+" , "+orientation[2]);
	}

	private void setLight() {
		this.sensor = sensorManager.getDefaultSensor(Sensor.TYPE_LIGHT);
		sensorManager.registerListener(this, this.sensor,
	...
	public float getLight() {
		 ...
		 * Log.i("getLight() -> battery = "
		 * ,""+this.batteryState+" , "+this.percent+"%");
		 * Log.i("getLight() -> ///////////", "/////////////////////////");
		 ...
		 * Log.i("getLight() -> ///////////", "/////////////////////////");
		 * Log.i("getLight() -> orientation",
		 * "x = "+this.sensor_orientation[0]);
		 * Log.i("getLight() -> orientation",
		 * "y = "+this.sensor_orientation[1]);
		 * Log.i("getLight() -> orientation",
		 * "z = "+this.sensor_orientation[2]);
		 */
		return this.light;
	}
	...
	public void stopListener() {
		...
		/*
		 * else { setLight(); setAccelerometer(); setMagneticField();
		 * setOrientation(); this.sensorlistener_flag = true; }
		 */
	}
	...
	private int isNetworkAvailable() {
		State gprs = connectManager.getNetworkInfo(
	...
	public void showinfo(Activity a) {
		...
		String str = "光线：" + this.light + ";\n" + "噪音：" + this.noise + ";\n"
				...
				+ this.sensor_orientation[1] + ";\n" + "z方向："
				+ this.sensor_orientation[2] + ";\n" + "电量：" + this.percent
				+ ";\n" + "时间：" + this.date + ";";
