...
public class Upload {
	...
	static public JSONObject Uploading(Activity app, String uploadUrl,
			JSONObject sendObj) {
		// HttpPost post = new HttpPost(uploadUrl);
		// HttpPost post = new HttpPost("http://10.108.108.11/uploadjson.php");
		HttpPost post = new HttpPost(
				"http://10.108.105.190:8080/uploadFile/fileServlet");
		...
		HttpConnectionParams.setConnectionTimeout(httpParameters, 1000);// Set
																		// the
																		// default
																		// socket
																		// timeout
	...
	static public void Uploading(Activity app, String uploadUrl, String fileName) {
		...
		try {
			 ...
			 * DataOutputStream(httpURLConnection.getOutputStream()); String
			 * content = "username="+URLEncoder.encode("forgetzzy", "utf-8");
	...
	static public JSONObject Uploading(String uploadUrl, JSONObject sendObj) {
		...
		httpParameters = new BasicHttpParams();// Set the timeout in
												// milliseconds until a
												// connection is established.
		HttpConnectionParams.setConnectionTimeout(httpParameters, 20000);// Set
																			// the
																			// default
																			// socket
																			// timeout
	...
	static public JSONObject Uploading(String uploadUrl, String fileName) {
		...
		try {
			...
			// URL url = new URL("http://10.108.108.11/upload11.php");
			// URL url = new
			// URL("http://10.108.105.190:8080/webInterface/fileServlet");
			Log.i("Upload", "file = " + fileName);
			HttpURLConnection httpURLConnection = (HttpURLConnection) url
					.openConnection();
			// ����ÿ�δ��������С��������Ч��ֹ�ֻ���Ϊ�ڴ治�����
