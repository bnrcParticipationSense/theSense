...
public class DataDAO extends DAOHelper {
	...
	public List<DataModel> findNotUploadDatas(Integer num, Context context) {
		...
		Long index = prefs.getLong(CommonDefinition.PREF_DATA_INDEX,
				CommonDefinition.PREF_DATA_INDEX_DEFAULT);

		try {
			SQLiteDatabase db = getReadableDatabase();
			cursor = db.query(DATA_TABLE_NAME, DATA_ALL_COLUMS, _ID + "> ?",
	...
	private DataModel createNewFile(SQLiteDatabase db, DataModel data) {
		...
		if (data.getLightIntensity() != null) {
			Logger.d("Creating new data for light " + data.getLightIntensity()
					+ " with a file create time of '"
					+ m_sdf.format(data.getCreateTime()) + "'");
		}
		if (data.getSoundIntensity() != null) {
			Logger.d("Creating new data for sound " + data.getSoundIntensity()
					+ " with a file create time of '"
					+ m_sdf.format(data.getCreateTime()) + "'");
		}
	...
	private DataModel createDataFromCursorData(Cursor cursor) {
		...
		Date createTime = new Date(cursor.getLong(3));
		; // 采集时间
		...
		return new DataModel(id, lightIntensity, soundIntensity, createTime,
				chargeState, batteryState, netState, longitude, latitude);
	}
}
