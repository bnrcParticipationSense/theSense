...
public class PreprocessingProcess extends PMProcess {
	...
	public PreprocessingProcess(List<PhotoData> photoData, Handler taskHandler, boolean isTrainingData)
	{
	...
	public void run()
	{
		...
		taskStatus = new PMTaskStatus(PMSettings.TASK_PREPROCESS, 0, progressStep, PMSettings.TASKRESULT_OK);
		sendMsgToHandler(taskStatus);
		
		requestPMData();

		taskStatus = new PMTaskStatus(PMSettings.TASK_PREPROCESS, 1, progressStep, PMSettings.TASKRESULT_OK);
		sendMsgToHandler(taskStatus);
		
		requestWeatherData();
		
		taskStatus = new PMTaskStatus(PMSettings.TASK_PREPROCESS, 2, progressStep, PMSettings.TASKRESULT_OK);
		sendMsgToHandler(taskStatus);
		
		// filter the data
		PhotoDataFilter pdf = new PhotoDataFilter(photoData,new FilterCriteria(), isTrainingData);
		isValidData = pdf.ValidateData();
		
		Log.i(LOG_TAG, LOG_PREFIX+"isValidData: " + photoData.get(0).getWeatherData().getWeather() + " "+ (isValidData?"yes":"no"));
		
		if(isValidData)
		{
			taskStatus = new PMTaskStatus(PMSettings.TASK_PREPROCESS, 3, progressStep, PMSettings.TASKRESULT_OK);
		}
		else
		{
			taskStatus = new PMTaskStatus(PMSettings.TASK_PREPROCESS, 3, progressStep, PMSettings.TASKRESULT_INVALID);
		}
	...
	public void requestPMData()
	{
		...
		try 
		{
			Thread.sleep(3000);
			/*
			JSONObject revJson = Upload.Uploading(PMConfig.pmWeatherURL, getPMRequest());
			updatePMData(revJson);
			*/
			
			// ONLY FOR TEST
			int testPmArray[] = {322,13,25,12,14,102,213,9,14,22,49,76,74,13,18,141};
	...
	public void requestWeatherData()
	{
		...
		try 
		{
			Thread.sleep(3000);
			/*
			JSONObject revJson = Upload.Uploading(PMConfig.pmWeatherURL, getWeatherRequest());
	...
	public void updateWeatherData(JSONObject jsonObj)
	{
		...
		try 
		{
			...
			if(responseType.equals(PMConfig.jsonRequestWeather))
			{
				if((Integer)jsonObj.get("response_num") == photoData.size())
				{
					JSONArray pmArray = (JSONArray)jsonObj.get("response_weahters");
					...
					for(int i = 0; i < photoData.size(); i++)
					{
						...
						photo.getWeatherData().setTemperature((Integer)pm.get("temperature"));
						photo.getWeatherData().setHumidity((Integer)pm.get("humidity"));
						photo.getWeatherData().setPressure((Integer)pm.get("pressure"));
						photo.getWeatherData().setWindspeed((Integer)pm.get("windspeed"));
						photo.getWeatherData().setPrecipitation((Integer)pm.get("precipitation"));
						photo.getWeatherData().setWeather(WeatherType.valueOf((Integer)pm.get("temperature")));
					}
				}
